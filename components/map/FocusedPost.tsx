...
export default function FocusedPost({ post }: { post: Post }) {
  return (
    <div className="max-w-xs container bg-white rounded-xl shadow-lg transform transition duration-500 hover:scale-105 hover:shadow-2xl">
      <div>
        ...
        <span className="text-white text-xs font-bold rounded-lg m-2 bg-red-500 inline-block py-1.5 px-4 cursor-pointer float-right">
          X
        </span>
      ...
      <img className="w-full cursor-pointer" src={post.images} alt="post_image" />
      <div className="flex p-4 justify-between">
        <div className="flex items-center space-x-2">
          <img
            ...
            alt="sara"
          />
          <h2 className="text-gray-800 text-sm cursor-pointer">{post.author?.email}</h2>
        </div>
