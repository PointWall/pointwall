...
function InputSection ({ children, title, helpText }: InputSectionProps): JSX.Element {
  return (
    <div className='relative border-b-2 border-slate-700 pb-4 last:border-b-0 md:border-b-0 md:border-l-4 md:pb-0 md:pl-4'>
      <h3 className='mb-2 text-xl'>
        {title}
        {
          helpText !== undefined &&
            <span className='md:relative ml-2 text-gray-400 rounded-full border w-5 h-5 inline-flex items-center justify-center hover:bg-gray-100 group'>
              <FontAwesomeIcon icon={faQuestion} className='block text-xs' />
              <span className='absolute w-56 left-0 bottom-full md:left-full ml-2 hidden group-hover:block text-xs bg-white border text-gray-800 rounded p-2 shadow-lg animate-fade-down md:animate-fade-right animate-duration-200 z-20'>{helpText}</span>
            </span>
        }
      </h3>
      {children}
        ...
        <section className='my-16 mb-8 accent-slate-700'>
          <Wrapper>
            {session?.user != null
              ? (
                <form onSubmit={handleSubmit}>
                  <div className='flex flex-col gap-6'>
                    <InputSection title='¿Qué tipo de manifestación artística querés compartir?'>
                      <div className='ml-2'>
              ...
              : (
                <div>
                  <p className='md:mt-32 mt-16 mb-12 text-center'>Iniciá sesión para poder contribuir</p>
                  <span className='block w-fit mx-auto md:border md:rounded-md'>
                    <LoginButton />
                  </span>
                </div>
                )}
